...
function Array_cycle(n,          // @arg Integer:
                     callback) { // @arg Function(= null):
                                 ...
                                 // @help: Array#cycle
//{@assert
    _if(_omit(n) || !_isInteger(n) || n < 0, "invalid Array#cycle(n)");
    _if(!_omit(callback) && !_isFunction(callback), "invalid Array#cycle(,callback)");
//}@assert
...
function Array_delete(value,      // @arg Any:
                      callback) { // @arg Function(= null):
                                  ...
                                  // @help: Array#delete
//{@assert
    _if(!_omit(callback) && !_isFunction(callback), "invalid Array#delete(,callback)");
//}@assert
...
//{@assert
    _if(!_isInteger(index), "invalid Array#delete_at(index)");
//}@assert
...
//{@assert
    _if(!_isFunction(callback), "invalid Array#delete_if(callback)");
//}@assert
...
function String_center(width,     // @arg Integer:
                       padding) { // @arg String(= " "):
                                  // @help: String#center
//{@assert
    _if(!_isInteger(width),                     "invalid String#center(width)");
    _if(!_omit(padding) && !_isString(padding), "invalid String#center(,padding)");
//}@assert
...
function String_gsub(pattern,   // @arg RegExp:
                     replace) { // @arg String/Function:
                                ...
                                // @help: String#gsub
//{@assert
    _if(!_isRegExp(pattern), "invalid Array#gsub(pattern)");
    _if(!_isString(replace) && !_isFunction(replace), "invalid Array#gsub(,replace)");
//}@assert
...
function String_ljust(width,     // @arg Integer:
                      padding) { // @arg String(= " "):
                                 ...
                                 // @help: String#ljust
//{@assert
    _if(!_isInteger(width),                     "invalid String#ljust(width)");
    _if(!_omit(padding) && !_isString(padding), "invalid String#ljust(,padding)");
//}@assert
...
function String_rjust(width,     // @arg Integer:
                      padding) { // @arg String(= " "):
                                 // @help: String#rjust
//{@assert
    _if(!_isInteger(width),                     "invalid String#rjust(width)");
    _if(!_omit(padding) && !_isString(padding), "invalid String#rjust(,padding)");
//}@assert
...
function String_sub(pattern,   // @arg RegExp:
                    replace) { // @arg String/Function:
                               ...
                               // @help: String#sub
//{@assert
    _if(!_isRegExp(pattern), "invalid Array#sub(pattern)");
    _if(!_isString(replace) && !_isFunction(replace), "invalid Array#sub(,replace)");
//}@assert
...
function String_scan(pattern,    // @arg RegExp/String:
                     callback) { // @arg Function(= null): callback(value:String/StringArray, index:Integer):void
                                 ...
                                 // @help: String#scan
//{@assert
    _if(!_isRegExp(pattern) && !_isString(pattern), "invalid String#scan(pattern)");
    _if(!_omit(callback) && !_isFunction(callback), "invalid String#scan(,callback)");
//}@assert
...
function Number_upto(max,  // @arg Integer:
                     fn) { // @arg Function: callback(num:Integer, index:Integer array:Array)
                           // @help: Number#upto
//{@assert
    _if(!_isInteger(max), "invalid Number#upto(max)");
    _if(!_isFunction(fn), "invalid Number#upto(,fn)");
//}@assert
...
function Number_step(limit, // @arg Integer:
                     ...
                     fn) {  // @arg Function: callback(num:Integer, index:Integer, array:Array)
                            // @help: Number#step
//{@assert
    _if(!_isInteger(limit), "invalid Number#step(limit)");
    _if(!_isInteger(step), "invalid Number#step(step)");
    _if(!_isFunction(fn), "invalid Number#step(fn)");
//}@assert
...
function Number_to_s(radix) { // @arg Integer(= 10): 2 - 36
                              // @help: Number#to_s
//{@assert
    _if(!_omit(radix) && !_isInteger(radix), "invalid Number#to_s(radix)");
//}@assert
...
//{@assert
function _omit(target) {
    return target === undefined;
}
function _isRegExp(target) {
    return target !== undefined && (Object.prototype.toString.call(target) === "[object RegExp]");
}
